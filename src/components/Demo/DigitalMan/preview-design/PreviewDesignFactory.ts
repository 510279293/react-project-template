...
import { TopToolFactory } from './TopToolFactory'
...
export class PreviewDesignFactory {
  ...
  removeItem(index) {
    try {
      ...
      // 找到对应需要删除数据的索引
      const delIndex = this.lists.findIndex(item => delShape.uuid === item[this.uuidKey])
  ...
  getTopToolPosition(shape) {
    if (!shape) {
      return { x: 0, y: 0 }
    }
    let width = this.getW(shape) * this.devicePixelRatio
    let height = (46 + 8) * this.devicePixelRatio
    let x = shape.x - this.paddingLeft - width;
    let y = shape.y
    let minY = this.paddingTop + 46 * this.devicePixelRatio
    let maxY = this.canvas.height - (46 + 8) * this.devicePixelRatio
  ...
  initSizeTextShape() {
    ...
    if (this.isShapePostion(shape) && this.isShapePostion(this.topToolShape)) {
      let xx = Math.round(shape.x / this.zoomRatio)
      let yy = Math.round(shape.y / this.zoomRatio)
      let ww = Math.round(shape.w / this.zoomRatio)
      let hh = Math.round(shape.h / this.zoomRatio)

      let gap = 4 * this.devicePixelRatio
      let width = 108 * this.devicePixelRatio
      let height = 46 * this.devicePixelRatio
  ...
  initActionTextShape() {
    ...
    let width = (this.lang === 'en_us' ? tipTexts[action].wEn : tipTexts[action].w)
    let options = {
      x: tipTexts[action].x,
      y: tipTexts[action].y,
      w: width,
  ...
  initShapes() {
    ...
    let shapeLen = this.shapeCollection.length
  ...
  async resetListsTimerFn(lists) {
    ...
    let shapeCollection = new Array(lists.length)
  ...
  async draw() {
    ...
    if (subtitleItemIndex >= 0) {
      let result = this.startDraw(subtitleItemIndex);
      if (result) {
        hasNewShape = true
      }
    }
  ...
  initLineShape() {
    ...
    if (shape) {
      ...
      let isCenterY = shape.x + shape.w / 2 >= conW / 2 - 1 && shape.x + shape.w / 2 <= conW / 2 + 1
      let isCenterX = shape.y + shape.h / 2 >= conH / 2 - 1 && shape.y + shape.h / 2 <= conH / 2 + 1
      let xOptions = {
        x: 0,
        y: shape.y + shape.h / 2,
        w: this.canvas.width,
  ...
  setSelectedShape(e, rect, clickX, clickY) {
  ...
  actionShape(shape, action) {
    ...
    if (action === 'copyImageShape') {
      let isCopy = this.onBeforeCopy(this.lists, shape)
      if (isCopy) {
   ...
   */
  copyShape(shape) {
    let newItem = Object.assign({}, this.lists[this.selectedIndex])
   ...
   * resize控件
   */
  resizeRect(e, clickX, clickY, rect, shape) {
    const { w, h } = shape
   ...
   * @param {*} h
   */
  initTopLeftHandler(shape, moveX, moveY, w, h) {
  ...
  setTopLeftRightSize(shape, moveY, fixedRatio) {
  ...
  initTopCenterHandler(shape, moveX, moveY, w, h) {
    let maxY = this.canvas.height - this.paddingBottom
  ...
  initTopRightHandler(shape, moveX, moveY, w, h) {
    const fixedRatio = h / w
  ...
  initLeftCenterHandler(shape, moveX, moveY, w, h) {
    if (shape.leftCenterHandler && shape.leftCenterHandler.selected) {
      if (moveX + this.shapeMinWidth < shape.maxX) {
        let maxX = this.canvas.width - this.paddingRight
  ...
  initRightCenterHandler(shape, moveX, moveY, w, h) {
  ...
  initBottomLeftHandler(shape, moveX, moveY, w, h) {
    const fixedRatio = h / w
  ...
  initBottomCenterHandler(shape, moveX, moveY, w, h) {
  ...
  initBottomRrightHandler(shape, moveX, moveY, w, h) {
    const fixedRatio = h / w
  ...
  setShapeStatus(shape, moveX, moveY) {
  ...
  resetShapeSelectd() {
    ...
    let selected = this.lists.find(item => item.selected)
